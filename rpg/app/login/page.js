  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-stone-950 p-4">
      <div className="rpg-card w-full max-w-md">
        <h1 className="text-3xl text-amber-500 text-center mb-6 font-serif">Pórtico de Entrada</h1>
        <div className="space-y-4">
          ...
          <button onClick={handleAuth} className="rpg-btn w-full">
            {isSignUp ? "Criar Personagem" : "Entrar no Reino"}
          </button>
          <p className="text-center text-xs text-stone-500 cursor-pointer" onClick={() => setIsSignUp(!isSignUp)}>
            {isSignUp ? "Já possuo uma conta" : "Sou novo nestas terras (Cadastrar)"}
          </p>
        </div>
      </div>
    </div>

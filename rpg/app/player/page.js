        ...
        <div className="rpg-card flex justify-between items-center border-amber-600 border-2">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right font-serif">
              <span className="text-yellow-400 block">💰 {profile.gold}g</span>
              <span className="text-blue-400 block">✨ {profile.xp}xp</span>
            </div>
            <button onClick={handleLogout} className="rpg-btn bg-red-900 border-red-950 text-[10px] px-3 py-1">Sair</button>
          </div>
        </div>
      )}
      ...
      <main>
          ...
          <div className="grid gap-4">
            {missions.map(m => (
              <div key={m.id} className="rpg-card border-l-4 border-l-amber-700">
                ...
                <p className="text-[10px] text-amber-700 font-bold uppercase">Premiação: {m.gold_reward}g | {m.xp_reward}xp</p>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {shop.map(item => (
              <div key={item.id} className="rpg-card">
                <p className="text-[10px] text-amber-600 font-bold">{item.seller_name} vende:</p>
                <h3 className="text-lg font-bold">{item.item_name}</h3>
                <button onClick={() => buyItem(item)} className="rpg-btn w-full mt-2 py-1 text-sm">Comprar por {item.price}g</button>
              </div>
            ))}
          </div>
        )}
        ...
        {tab === 'players' && (
          <div className="rpg-card">
            <h2 className="text-xl text-amber-200 mb-4 font-serif text-center">Outros Aventureiros</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {allPlayers.map(p => (
                <div key={p.username} className="p-2 border border-amber-900/30 bg-stone-900 rounded text-center">
                  ...
                  <p className="text-[9px] text-stone-500">✨{p.xp} | 💰{p.gold}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab === 'inv' && (
          <div className="rpg-card">
            ...
            <div className="grid grid-cols-5 gap-3">
              {[...Array(10)].map((_, i) => (
                <div key={i} className="aspect-square border-2 border-amber-900/40 bg-stone-950 rounded flex items-center justify-center p-1 text-center">
                  {profile?.inventory?.[i] ? (
                    <span className="text-[8px] text-amber-100 font-bold">{profile.inventory[i]}</span>
                  ) : (
